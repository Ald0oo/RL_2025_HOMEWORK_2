...
public:
  ...
  Ros2KdlNode() : Node("ros2_kdl_node") {
    // Inizializza l'Action Server
...
private:
  ...
  // Callbacks di gestione della richiesta
  rclcpp_action::GoalResponse handle_goal(
  const rclcpp_action::GoalUUID & uuid,
  std::shared_ptr<const ExecuteTrajectory::Goal> goal)
{
  RCLCPP_INFO(this->get_logger(), "Goal ricevuto!");
  (void)uuid;
  (void)goal; // Aggiungi questa riga!
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Annullamento richiesto dal client.");
    (void)goal_handle;
  ...
  {
    // Esegui la traiettoria in un thread separato per non bloccare ROS 2
    std::thread{std::bind(&Ros2KdlNode::execute, this, std::placeholders::_1), goal_handle}.detach();
  }

  // Loop di controllo e invio feedback
  void execute(const std::shared_ptr<GoalHandleExecuteTrajectory> goal_handle)
  {
    ...
    double t = 0.0;
    double dt = 0.01; // Step temporale
    double total_time = this->get_parameter("total_time").as_double();

    while (t < total_time && rclcpp::ok()) {
      // Verifica se il client ha richiesto di annullare
      ...
      // 1. Calcola punto della traiettoria desiderata in funzione di t
      // 2. Calcola legge di controllo cinematica (KDL)
      // 3. Pubblica le velocità comandate ai giunti del robot
      
      // 4. Calcola l'errore di posizione norm(e_p)
      double current_pos_error =0.0; // norma dell'errore cartesiano ep

      // 5. Pubblica il feedback all'Action Client
      feedback->position_error = current_pos_error;
    ...
    // A traiettoria completata con successo
    if (rclcpp::ok()) {
      result->success = true;
      goal_handle->succeed(result);
      RCLCPP_INFO(this->get_logger(), "Traiettoria completata con successo!");
    }
  }
};
...
this->declare_parameter<double>("trajectory_len", 0.4);
this->declare_parameter<double>("Kp", 10.0);
this->declare_parameter<std::vector<double>>("end_position", {0.5, 0.2, 0.4});

// Recupero valori
double traj_duration = this->get_parameter("traj_duration").as_double();
std::vector<double> end_pos = this->get_parameter("end_position").as_double_array();
